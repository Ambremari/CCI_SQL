...
CREATE TABLE Clients(
  IdCl decimal(8)
CONSTRAINT PK2_Clients PRIMARY KEY,
...
CONSTRAINT Dom_MailCl CHECK (MailCl LIKE ('%@%.%')),
  TelCl Char(10)
CONSTRAINT Dom_TelCl CHECK (TelCl LIKE('0%')),
  PointsFid decimal(4),
  DateNaisCl Date
CONSTRAINT Dom_DateNaisCl CHECK (DateNaisCl > TO_DATE('1900-01-01', 'yyyy-mm-dd')),
  DateAdhCl Date
CONSTRAINT Dom_DateAdhCl CHECK (DateAdhCl > TO_DATE('2000-01-01', 'yyyy-mm-dd')),
  CPCl decimal(5)
...
CONSTRAINT PK_Commandes PRIMARY KEY,
  DateCmd Date
CONSTRAINT Dom_DateCmd CHECK (DateCmd > TO_DATE('2000-01-01','yyyy-mm-dd')),
  EtatCmd Char Varying(30) DEFAULT 'En attente de validation' NOT NULL
...
CONSTRAINT Dom_DateLiv_DateCmd CHECK (DateLiv > DateCmd)
);
